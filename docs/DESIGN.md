...
- **Two known causes of a doubled or vanishing name, neither fixed yet.** A skin keeps a
  second, much narrower name field carrying a `UISystem.LocalizeText.BindingText` bound to
  `Duel.Player.Name`, so it repaints itself from the model whatever is done to the caption —
  Standard's `Duelist/Text_obj` is 0.56 units wide against the caption's 3.76, exactly wide
  enough to show `Vl` of `Vlada`. And game code owns duelists 1 and 2 and repaints their
  caption from its own state, so a name written there during the settle window is put back as
  soon as the window closes. Fixes for both were written and **reverted** — they were never
  seen on the phone and coincided with names flickering on and off frame by frame. See
  `userstories/US-04`, "Treperenje". **Get a log before writing either one again.**
